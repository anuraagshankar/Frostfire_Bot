/*
	Depth 4 bot vs Depth 3 grouping Bot
*/
...
class botG
{
	friend void playAgainstBot(botG b);
	void botvsbot(bot b1, botG b2);
	void botvsbot(botG b1, bot b2);
	friend void play();
...
botG::botG(Board c, int mD, int bP)
{
	comp = c;
	maxDepth = mD;
	botPlayer = bP;
...
vector<string> botG::getMarbles(int color)
{
	vector<string> allMarbles;
	for(unordered_map<string, Marble>::iterator it = comp.marbles.begin(); it != comp.marbles.end(); it++)
	{

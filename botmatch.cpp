...
class abalone
{
	...
	friend void botMatch(bot a, bot b);
	// data members
...
bot::bot(abalone a, int bP, int mD)
{
	comp = a;
	botPlayer = bP;
	user = !bP;
...
vector<int> bot::minimax(int depth, bool isMax)
{
	...
	else if(depth == maxDepth) return {value, depth};
...
void botMatch(bot b1, bot b2)
{
	...
	while(a.winner() == -1)
	{
		...
		count = 0;
		string s1 = compMove.substr(0, 2), s2 = compMove.substr(3, 2);
		vector<vector<vector<int>>> coor = a.getCoordinates(s1, s2);
		a.move(coor[0], coor[1]);
		a.displayGame();
		a.activePlayer = !a.activePlayer;
		...
		cout << "Time taken: " << duration.count() << "ms" << endl;
		count = 0;
		s1 = compMove.substr(0, 2); s2 = compMove.substr(3, 2);
		coor = a.getCoordinates(s1, s2);
		a.move(coor[0], coor[1]);
		a.displayGame();
		a.activePlayer = !a.activePlayer;
